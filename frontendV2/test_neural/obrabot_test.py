...
from update_weekend.anylaze import WeekendAnalyzer as WF
...
class DataProcessor:
    ...
    def write_to_csv(self, forecast, filename, filename2):
        ...
        data1 = pd.read_csv(filename2, delimiter=';')
...
def start_obrabot_test(filename, latitude, longitude, hours):
    try:
        with open(f'files/test/{filename}', newline='', encoding='utf-8') as csv_file:
        ...
        processor = DataProcessor(f'files/test/{filename}', latitude, longitude, start_date, end_date)
        processor.load_data()
        weather_forecast = processor.get_weather_data()
        processor.write_to_csv(weather_forecast, f'files/result/{filename}', f'files/test/{filename}')
        
        pr = WF(f'files/result/{filename}', f'files/result/{filename}')
        pr.analyze_pred(hours, filename)
