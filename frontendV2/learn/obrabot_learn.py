...
def start_obrabot_learn(filename, latitude, longitude):
    try:
        with open(f'files/update/{filename}', newline='', encoding='utf-8') as csv_file:
        ...
        processor = DataProcessor(f'files/update/{filename}', latitude, longitude, start_date, end_date)
        processor.load_data()
        weather_forecast = processor.get_weather_data()
        processor.write_to_csv(weather_forecast, f'files/learn/l_{filename}', f'files/update/{filename}')
        
        pr = WF(f'files/learn/l_{filename}', f'files/learn/l_{filename}')
        pr.analyze_test()
